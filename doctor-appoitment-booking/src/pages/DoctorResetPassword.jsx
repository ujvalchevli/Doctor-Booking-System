...
function DoctorResetPassword() {
  ...
  const sendOtp = async (e) => {
    ...
    setIsLoading(true);
    try {
      const { data } = await axios.post(backendUrl + "/api/doctor/send-reset-otp", {
        email,
      });
      if (data.success) {
          ...
          <form onSubmit={sendOtp}>
            <div className="text-center mb-8">
              ...
              <p className="text-gray-500 text-sm">
                Enter your professional email to receive an OTP
              </p>
            </div>
            ...
            >
              <FiArrowLeft className="mr-1" /> Back to Doctor Login
            </button>
          </form>
        ) : (
